...
def Avg_pool_3d(target, kernel_size = 3, strides = 1, name='avg'):
    return tflearn.layers.conv.avg_pool_3d(target, kernel_size=kernel_size, strides=strides, padding="same", name=name)
...
def convolveBN(opName, inputLayer, inputChannel, outputChannel, kernelSize, stride, stddev=1e-2):
    return batch_normalization(tflearn.layers.conv_2d(inputLayer, outputChannel, kernelSize, strides=stride,
                                  padding='same', activation='linear', bias=True, scope=opName))
...
def convolveBNLeakyReLU(opName, inputLayer, inputChannel, outputChannel, kernelSize, stride, alpha=0.1, stddev=1e-2):
    ...
    def __call__(self, net, args, opt=None):
        ...
        for i in self.numGPU:
            ...
            with tf.device(auto_gpu):
                ...
                net.controller = self
                result = net(*[arg[j] for arg in args])
