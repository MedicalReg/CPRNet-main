...
parser.add_argument('-g', '--gpu', type=str, default='2,3',
                    help='Specifies gpu device(s)')
parser.add_argument('-c', '--checkpoint', type=str, default= None,
                    help='Specifies a previous checkpoint to start with')
parser.add_argument('-d', '--dataset', type=str, default="./datasets/brain-1.json",
...
os.environ['CUDA_VISIBLE_DEVICES'] = '3,7'#args.gpu
...
def main():
    ...
    with tf.Session(config=config) as sess:
        saver = tf.train.Saver(tf.get_collection(
            tf.GraphKeys.GLOBAL_VARIABLES), max_to_keep=5, keep_checkpoint_every_n_hours=1)
        ...
        if args.checkpoint is None:
            ...
            if args.clear_steps:
                ...
                steps = int(re.search('model-(\d+)', checkpoints).group(1))
